...
def is_gemini3_model(model_id: str) -> bool:
    return bool(
        re.search(r"gemini[-_]?3(?:\.[-_]?1)?", model_id, flags=re.IGNORECASE)
    )
...
def is_glm_model(model_id: str) -> bool:
    return bool(
        re.search(r"glm-(?:5|4\.(?:6|7))(?!\d)", model_id, flags=re.IGNORECASE)
    )
...
def is_qwen35_one_million_context_model(model_id: str) -> bool:
    return bool(
        re.search(r"qwen[-_]?3(?:\.|[-_])?5[-_]?(?:flash|plus)", model_id, flags=re.IGNORECASE)
    )
...
def is_claude_opus_46_model(model_id: str) -> bool:
    return bool(re.search(r"claude[-_]?opus[-_]?4(?:\.|-)6", model_id, flags=re.IGNORECASE))
...
def is_ming_flash_omni_model(model_id: str) -> bool:
    return bool(
        re.search(r"ming[-_]?flash[-_]?omni[-_]?2(?:\.|-)0", model_id, flags=re.IGNORECASE)
...
def resolve_global_token_limits(
    ...
    *,
    ...
    if is_gemini2_model(model_id):
        return TokenLimits(GEMINI2_MAX_INPUT_TOKENS, GEMINI2_MAX_OUTPUT_TOKENS)

    if is_gemini25_model(model_id):
        return TokenLimits(GEMINI25_MAX_INPUT_TOKENS, GEMINI25_MAX_OUTPUT_TOKENS)

    if is_gemini3_model(model_id):
        return TokenLimits(GEMINI3_MAX_INPUT_TOKENS, GEMINI3_MAX_OUTPUT_TOKENS)

    if is_glm_model(model_id):
        return TokenLimits(FIXED_256K_MAX_INPUT_TOKENS, FIXED_256K_MAX_OUTPUT_TOKENS)

    if is_glm45_model(model_id):
        return TokenLimits(128 * TOKENS_PER_KIBI - 32 * TOKENS_PER_KIBI, 32 * TOKENS_PER_KIBI)
    ...
    if is_minimax_model(model_id):
        return TokenLimits(
            200 * TOKENS_PER_KIBI - 32 * TOKENS_PER_KIBI,
            32 * TOKENS_PER_KIBI,
        )
    ...
    safe_context_length = (
        ...
        and context_length > reserved_input_tokens
...
def resolve_advertised_token_limits(
    ...
    *,
    ...
    resolved_limits = resolve_global_token_limits(
        model_id,
        advertised_context_length if advertised_context_length is not None else default_context_length,
...
def resolve_global_capabilities(
    ...
    *,
    ...
    return {
        "toolCalling": True if detected_tool_calling is None else bool(detected_tool_calling),
